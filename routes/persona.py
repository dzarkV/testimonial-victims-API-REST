...
@persona.get(
    ...
    tags=["personas"],
    description="Obtiene las personas de los testimonios del Libro",
)
async def find_all_people(
    skip: int = Query(
        1, gt=0, le=42, description="Limite mínimo de todas las personas"
    ),
    limit: int = Query(
        42, gt=0, le=42, description="Limite máximo de las personas en los testimonios"
    ),
) -> list:
    allPeople = personasEntity(
        collection.find(
            {"id": {"$gte": skip, "$lte": limit}}, {"id": 1, "value.persona": 1}
        ).sort("id")
    )
    return allPeople
